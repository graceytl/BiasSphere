...
class HolisticAIBedrockChat(BaseChatModel):
    ...
    api_endpoint: str = Field(
        default="https://ctwa92wg1b.execute-api.us-east-1.amazonaws.com/prod/invoke", description="API endpoint URL"
    )
    ...
    model: str = Field(default="us.anthropic.claude-3-5-sonnet-20241022-v2:0", description="Model identifier")
            ...
            if isinstance(msg, SystemMessage):
                ...
                if hasattr(msg, "tool_calls") and msg.tool_calls:
                    ...
                    for tc in msg.tool_calls:
                        ...
                        content_blocks.append({"type": "tool_use", "id": tc_id, "name": tc_name, "input": tc_args})
                ...
                tool_result_content = [
                    {"type": "tool_result", "tool_use_id": tool_call_id, "content": str(msg.content)}
                ]
    ...
    def with_structured_output(
        ...
        **kwargs: Any,
        ...
        """
        return HolisticAIBedrockStructuredOutput(base_model=self, schema=schema, **kwargs)
        ...
        if tools:
            ...
            for tool in tools:
                ...
                if (
                    ...
                    and callable(getattr(tool, "name", None)) == False
                ):  # name should not be callable
                    try:
                        tool_dict = {
                            "name": tool.name,
...
class HolisticAIBedrockStructuredOutput:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._response_format = {
            "type": "json_schema",
            "json_schema": {"name": schema.__name__.lower(), "strict": True, "schema": cleaned_schema},
        }
    ...
    def invoke(self, input: Any, config: Optional[Any] = None, **kwargs: Any) -> PydanticBaseModel:
        ...
        if last_ai_content:
            ...
            messages_to_use = [
                HumanMessage(content=f"Format the following information as structured JSON:\n\n{last_ai_content}")
            ]
        else:
        ...
        try:
            result = self.base_model._generate(
                messages=messages_to_use, response_format=self._response_format, **kwargs
            )
