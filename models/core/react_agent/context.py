...
class Context:
    ...
    ollama_temperature: float = field(
        default=0.1,
        metadata={"description": "Temperature for Ollama model (0.0 to 1.0)"},
    )

    ollama_timeout: int = field(
        default=60,
        metadata={"description": "Timeout for Ollama requests in seconds"},
    )

    ollama_num_predict: int = field(
        default=256,
        metadata={"description": "Maximum number of tokens to predict"},
    )
