...
def load_chat_model(model_name: str, context=None) -> BaseChatModel:
    ...
    try:
        ...
        if context is not None:
            temperature = getattr(
                context, "temperature", context.ollama_temperature if hasattr(context, "ollama_temperature") else 0.7
            )
        ...
        if model_name in gpt5_models:
            ...
            if not openai_api_key:
                raise ValueError(f" GPT-5 model '{model_name}' requires OPENAI_API_KEY in .env file")
